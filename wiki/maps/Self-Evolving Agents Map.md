...
summary: >-
  自进化 agent 地图按学习层次组织已实现系统，并用验证器独立性、现实接地和更新范围判断改进能否安全持久。
provenance:
  extracted: 0.68
  inferred: 0.27
  ambiguous: 0.05
...
学习层级回答“系统改什么”，但不回答“凭什么相信改完更好”。[[wiki/concepts/Verifier Hierarchy]] 把验证信号按独立性和现实接地程度展开：自评、模型裁判、人类判断、环境执行和形式验证分别有不同强度与盲区。

这为地图里的系统增加了一组横向比较：

| 学习对象 | 常见验证方式 | 核心风险 |
|---|---|---|
| 运行时反思 / memory | 自评、任务重试、后续成功率 | 错误反思被固化、旧事实污染当前状态 |
| 单个 skill / prompt | 成对执行、生成任务、held-out gate | evaluator 偏好文风、任务分布过窄 |
| Skill repository | 跨任务下游奖励 | 延迟信用分配、局部改动污染共享仓库 |
| Harness code | 端到端 benchmark、raw traces、外部执行 | 改善 benchmark 同时扩大安全和权限风险 |
| Model weights | 大规模 held-out eval、真实环境反馈 | reward hacking、模式退化、评估器被共同优化 |

更新越持久、作用域越广、越难回滚，越不能只靠同一模型的自我评价批准。 ^[inferred]

这也是 [[wiki/syntheses/AI Agent Three Paradoxes|AI Agent 三重悖论]] 对本地图的修正：自进化不是把 proposer 做得越来越强，而是让 proposer、executor、verifier 和 governor 之间形成可反驳、可回滚的闭环。 ^[inferred]
