...
- [[wiki/sources/Use Case 协作与追踪矩阵 Source Guide]] — 说明产品、业务、RD、QA 和设计如何围绕同一个用例补充领域模型、UI、API、测试与开发任务追踪矩阵。
...
- [[wiki/sources/Grokking Simplicity Taming FP Source Guide]] — 保留《Grokking Simplicity》函数式编程读书笔记，把 action、calculation、data、timeline 和洋葱架构连接到复杂度治理。
- [[wiki/sources/Galls Law Source Guide]] — 保留 Gall’s Law 对 MVP、monolith first、渐进复杂度和避免过早微服务化的论证。
- [[wiki/sources/Second-System Effect Source Guide]] — 保留 Second-System Effect 对成功第一版之后过度设计、功能膨胀和大重写风险的警告。
- [[wiki/sources/Zawinskis Law Source Guide]] — 保留 Zawinski's Law 对 feature creep、平台化、工具焦点稀释和软件膨胀循环的警告。
- [[wiki/syntheses/信息流与状态流转设计原则]] — 整理软件运行时结构中的信息传播、状态迁移、幂等、一致性、事件驱动和补偿模式。

## Related Syntheses

- [[wiki/syntheses/类和对象结构图的建模作用]] — 类和对象结构图把领域概念、责任边界、对象协作和可演化设计连接起来。
