...
title: Hot Cache
category: meta
summary: 营养素是人体获得的物质，热量是可供能成分携带的能量；人体通过 ATP、糖原和脂肪形成即时、短期与长期储能层级。
tags: []
sources: []
created: 2026-05-04
...
- 基本世界观现在加入“偏好之外的回应”线索：喜欢只能说明什么吸引自己，不能独自决定什么重要；节日、承诺和纪念让过去、他者与有限性重新进入当下，但回应必须通过责任半径和反操纵判准守住主体性。
- 分类与结构化思维现在加入逻辑树/金字塔线索：MECE 检查横向分组，逻辑树外化问题分解，证据验证负责剪枝，金字塔把验证后的材料收成面向读者的核心主张。
- Agent harness 现在加入 AGENTS.md 操作契约线索：常驻上下文只保存高代价、不可轻易发现的仓库决策知识；局部规则按目录下沉，多步骤流程按需加载，机器可判定的硬约束进入自动化治理。
...
- 逻辑树和金字塔原理不是同义工具：前者的根节点通常是问题、对象或目标，允许保留待验证假设；后者的根节点是核心结论，要求下层理由和证据真正支撑主张。完整路径是“问题定义→逻辑树拆解→证据验证→金字塔交付”。
- AGENTS.md 的质量不由篇幅决定，而由它是否用最少常驻上下文减少探索、失败命令、架构越界和验证遗漏决定；文档负责让 Agent 知道，自动化机制负责让系统不会错。
