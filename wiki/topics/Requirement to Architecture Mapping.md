...
## 《架构师启示录》的补充：需求矩阵作为架构输入

《架构师启示录》把需求先放进二维矩阵：纵向区分业务需求、系统需求、用户需求，横向区分功能性需求、非功能性需求和约束条件。这个矩阵补充了本页的 normalized input：在进入领域对象、用例、模块职责和数据流之前，先要确认每一层需求中哪些是功能，哪些是质量属性，哪些是不可绕开的约束。

其中非功能性需求尤其关键。功能性需求说明系统做什么；非功能性需求和约束条件往往决定系统架构，例如性能、安全、高可用、可维护性、部署环境、团队能力和上线时间。

因此，从需求到架构的映射可以增加一个前置检查：
...
这会减少一种常见错误：只把需求当作功能清单，然后在实现后期才发现真正决定架构的是吞吐量、可靠性、合规、团队能力或数据归属。
