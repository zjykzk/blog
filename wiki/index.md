...
- [[wiki/concepts/Agent Transformer|Agent Transformer]] — Agent Transformer models an AI agent as a transformer policy embedded in memory, tools, verifiers, and environment feedback rather than as a standalone text model. (#agents #architecture #harness)
...
- [[wiki/concepts/Agent Evaluation Metric Vector|Agent Evaluation Metric Vector]] — Agent Evaluation Metric Vector treats agent evaluation as a trajectory-level vector of success, cost, tool correctness, robustness, safety, and intervention metrics. (#agents #evaluation #safety)
...
- [[wiki/concepts/Loop Engineering|Loop Engineering]] — Loop Engineering stacks execution, verification, event-triggering, and trace-driven improvement loops around an agent so value compounds beyond one model call. (#agents #harness #feedback #workflow)
- [[wiki/concepts/Event-Driven Agent Loop|Event-Driven Agent Loop]] — An event-driven agent loop connects an agent to webhooks, schedules, channels, or other ecosystem events so it runs as a background system component. (#agents #harness #workflow)
...
- [[wiki/concepts/幂律分布与正态分布]] — 幂律分布与正态分布的核心差别在于：正态分布让中心解释世界，幂律分布让尾部和极端值解释世界。 (#probability #systems-thinking #mechanism)
...
- [[wiki/concepts/姓氏名与字]] — 姓、氏、名、字分别对应血缘祖源、宗族分支、个人本名和成年社会称呼；姓氏合流反映宗法贵族秩序向国家户籍秩序转变。 (#history #culture #identity #mechanism)
...
- [[wiki/concepts/Agentic Coding Unknowns|Agentic Coding Unknowns]] — Agentic coding unknowns are the gaps between a user's prompt-map and the real codebase-territory that an agent must either clarify or guess through. (#agents #ai-coding #workflow #context)
...
- [[wiki/sources/AI Agent Systems Architectures Applications and Evaluation Paper Source Guide|AI Agent Systems Architectures Applications and Evaluation Paper Source Guide]] — Source guide for arXiv 2601.01743 on AI agent architectures, learning mechanisms, deployment domains, infrastructure, evaluation metrics, and open research directions. (#paper #arxiv #agents #architecture)
...
- [[wiki/sources/AI 什么都能写与软件价值重心转移 Source Guide|AI 什么都能写与软件价值重心转移 Source Guide]] — 保留一篇中文长文：AI 让实现、迁移和重写成本下降，软件价值重心转向知识、规格、harness、判断和责任。 (#article #agents #ai-coding #software-engineering #judgment)
...
- [[wiki/sources/Agentic Coding Unknowns Source Guide|Agentic Coding Unknowns Source Guide]] — Thariq Shihipar's Fable article frames agentic coding as iterative unknown discovery across prompt, plan, implementation, review, and buy-in artifacts. (#article #agents #ai-coding #workflow #context)
...
- [[wiki/sources/The Art of Loop Engineering Source Guide]] — LangChain 文章把生产级 agent 价值归因于四层循环叠加：agent 执行、验证、事件触发和基于 traces 的外层改进。 (#article #agents #harness #feedback #workflow)
...
- [[wiki/sources/Zawinskis Law Source Guide]] — Source guide preserving Zawinski's Law as a warning that successful applications tend to expand into all-purpose platforms through feature creep. (#book #software-engineering #architecture #systems)
