...
- `references/human-llm-collaborative-planning.md`：做人机协同规划、多 Agent 计划 DAG、process supervision 相关论文倒读时，可复用其中的候选溯源链、叙事框架和诚实标注 caveat。
...
7. *新论文不硬造下游* — 如果目标论文刚发布，找不到明确后续引用，就写“截至生成日期未找到直接后续”，再转向相邻前沿和最可能的下一步；不要为了补齐“前沿延伸”伪造引用链。
