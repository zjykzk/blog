...
如果用户在论文笔记完成后追问某个洞见句，例如“怎么把 X 拆到 Y 粒度”，不要只复述论文结论。把它转成可执行方法论：先说明这句话在解决什么类问题，再给一条可复用链路。

对“把要看的证据拆到能问、能答、能追责的粒度”，使用这条最小链路：

> 大判断 → 质量维度 → 原子问题 → 证据锚点 → 是非答案 → 错因类型 → 修复动作

执行要点：

1. 找总判断：例如“这个需求清楚吗”“这段代码质量好吗”“这个回答靠谱吗”。
2. 拆质量维度：事实、理解、推理、证据、边界、行动；或按具体领域换成需求、接口、状态、错误处理、可维护性等。
3. 维度变原子问题：一个问题只检查一个判断点，最好能答“是 / 否 / 不确定”。
4. 给问题挂证据锚点：说明去材料中的哪里看、比较什么、如何判定。
5. 失败要给错因类型和修复动作：不是“质量不好”，而是“缺少验收标准，导致工程无法判断完成条件；补成功/失败标准”。
6. 停止条件：问题答案会改变总判断、能被证据回答、失败后能导出修法。否则这个问题太虚或太碎。

短句：总评变维度，维度变问题，问题挂证据，失败给修法。
...
如果用户同时触发了别的相邻 skill，但输入明确是学术论文（arXiv / PDF / DOI / OpenReview / ACL / PMLR），以材料类型为准，继续用 ljg-paper；最终报告里简短说明“这是论文，所以按 ljg-paper 处理”。
...
**arXiv 元数据优先走 API 校准**：arXiv 论文先查 `https://export.arxiv.org/api/query?id_list={paper-id}`，用 Atom feed 校准 title / authors / published / abstract / categories。不要只信 TeX 源里的 `\title{...}`：source package 里常残留多个候选标题、旧摘要或注释草稿；最终元数据以 arXiv API 和 PDF 首页为准，TeX 源主要用来补 caption、实验描述和数字。
